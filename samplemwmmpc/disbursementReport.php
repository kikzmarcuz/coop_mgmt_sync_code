...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if($searchReport == "searchReport" or $printReport == "printReport"){
        ...
        //Financial Cost
        $sqlName = "SELECT * FROM  chart_account_entry_table WHERE expenses_type = 'FC' ";
        ...
        if($resultName->num_rows > 0){
            while ($row = mysqli_fetch_array($resultName)) {
                ...
                if($startDate <= $releaseDate and $endDate >= $releaseDate){
                    ...
                    $voucherNumber[$counter] = $row['reference_number'];
        ...
        //Administrative Cost
        $sqlName = "SELECT * FROM  chart_account_entry_table WHERE expenses_type = 'AC' ";
        $resultName = $conn->query($sqlName);
